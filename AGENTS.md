...
ADRs live in `docs/adr/`, one numbered Markdown file per significant decision.
New ones follow the org-wide template in
[mondoohq/.github](https://github.com/mondoohq/.github/blob/main/docs/adr/adr-template.md):
...
## Context
## Decision
## Security implications      <- required, never deleted
## Performance implications   <- required, never deleted
## Consequences               (Positive / Negative / Follow-up)
## Alternatives considered
## References
```

Security and Performance sections are **required**. If a decision genuinely
does not touch either, write "None — this decision does not change the threat
model" (or "no measurable impact expected") and say why. Deleting the section
is not an option.

**The log is append-only.** A superseded decision gets a new ADR that
references it; existing ADRs are not rewritten. `docs/adr/0001` through `0006`
here predate the org template and use an older `# ADR-NNNN:` heading — leave
them as they are rather than retrofitting.

Content rules: no secrets, internal hostnames, customer names, or details of an
...
All verified against the compiler and explained in full in [`content/AGENTS.md`](content/AGENTS.md). Repeated here so a reviewer that never opens that file still doesn't file the false positive.
...
| A dotted sub-resource path called a bare resource because the schema lists it as a resource | Since [mql#10349](https://github.com/mondoohq/mql/pull/10349) the walk stops at the owner when the sub-resource is `private` **and** the owner declares a non-implicit field of that name with exactly that type. `aws.emr.cluster.encryptionConfiguration.*`, `cloudflare.zone.settings.*`, `gcp.project.gkeService.cluster.networkPolicy.*` and `azure.subscription.aksService.cluster.autoUpgradeProfile.*` all compile as field reads now. Check both halves in the schema; the path being a resource name proves nothing by itself. |
...
Two bare boolean fields joined with `&&` therefore pass when neither resolved. Doesn't extend to comparisons: `null == "x"` is `false`, not null, so `field_a == "x" && field_b == "y"` fails when both are absent. Flag the bare-field form, leave the comparison form alone. The comparison form has its own asymmetry (`field != "insecure"` passes when the field is absent), covered in `content/AGENTS.md`.

A dotted path that is also a resource name is not *always* a field read. The compiler extends the resource path greedily, and where the longer resource wins the query builds a bare sub-resource whose accessor never runs; every field reads `null` and the check answers confidently wrong. [mql#10349](https://github.com/mondoohq/mql/pull/10349) narrowed this: the walk now stops at the owner when the sub-resource is `private`, the owner declares a field of that name, that field is not an implicit resource, and its type is exactly that resource. Miss any one and the resource still wins. The live shape is the type mismatch, where the owner's field of that name is a `dict`, a list, or a scalar: `gitlab.user.email` reads as a string field and compiles to the resource. Confirm from the schema, not from a hunch: `cnspec providers resources <provider> <full.dotted.path> --json` for `"private"`, then the same command on the owner for a field of that name and type. Note that a released `cnspec` 13.x binary still has the old compiler, so a local `--ast` run can disagree with CI. Full treatment, the current example list, and the fix are in `content/AGENTS.md`.
