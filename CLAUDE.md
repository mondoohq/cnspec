...
**Never hand-edit** the JSON files in `content/validation/cmd_data/`. They are generated by the dump scripts below. To add a new AWS service, add it to the `SERVICES` list in `dump_aws_commands.py` and regenerate. The gcloud script auto-detects services from the policy file.

**Regenerate command data** (needed when CLI versions change or new services are added):
