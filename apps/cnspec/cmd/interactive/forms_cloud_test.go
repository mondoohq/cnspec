...
func TestCloudACredentialsReachTheProvider(t *testing.T) {
	const secret = "must-never-reach-argv"
...
func TestNutanixCarriesEitherCredentialAndStillOffersThePrompt(t *testing.T) {
	...
	fill := func(flag string) form {
		...
		f.Fields()[i].SetValue("must-never-reach-argv")
		return f
	}
	...
	for _, flag := range []string{"password", "api-key"} {
		...
		assertCredentialReachesTheProvider(t, c, f, flag, "must-never-reach-argv")
	}
...
func TestCloudBCredentialsCanBeDelivered(t *testing.T) {
	for _, name := range cloudBConnectors {
		...
		for _, flag := range secretFlags {
			...
			filled.Fields()[i].SetValue("must-never-reach-argv")

			if deliveryFor(filled) == deliverPlain {
				t.Errorf("%s: --%s is not being treated as a secret at all", name, flag)
				continue
			}
			assertCredentialReachesTheProvider(t, c, filled, flag, "must-never-reach-argv")
			if strings.Contains(strings.Join(filled.Args(), " "), "must-never-reach-argv") {
...
func TestOCIKeySecretIsCarriedAsACredential(t *testing.T) {
	...
	assertCredentialReachesTheProvider(t, c, f, "key-secret", "must-never-reach-argv")
}
...
func TestProxmoxInventoryIsTheProvidersOwnReading(t *testing.T) {
	...
	set("token", "PVEAPIToken=root@pam!ui=must-never-reach-argv")
	...
	if got, _ := p.sentValue("token"); got != "PVEAPIToken=root@pam!ui=must-never-reach-argv" {
...
func TestAskPassStaysAToggleForVCDAndVsphere(t *testing.T) {
	for _, name := range []string{"vcd", "vsphere"} {
		...
		assertCredentialReachesTheProvider(t, c, f, "password", "must-never-reach-argv")
