...
func TestNdSSHPromptsForWhatItCannotCarry(t *testing.T) {
	...
	f.Fields()[f.IndexOfFlag("password")].SetValue("must-never-reach-argv")
	f.Fields()[f.IndexOfFlag("ask-enable-password")].SetOn(true)

	p := assertCredentialReachesTheProvider(t, c, f, "password", "must-never-reach-argv")
...
func TestAHiddenCredentialDoesNotDecideTheRoute(t *testing.T) {
	...
	f.Fields()[f.IndexOfFlag("password")].SetValue("typed-then-abandoned")

	// The user changes their mind.
	setLabelled(t, &f, "sign in with", "API key")
	f.Fields()[f.IndexOfFlag("api-key")].SetValue("the-real-key")
	...
	if got, _ := p.sentValue("api-key"); got != "the-real-key" {
	...
	for _, s := range append(append([]string{}, plan.args...), plan.env...) {
		if strings.Contains(s, "typed-then-abandoned") {
