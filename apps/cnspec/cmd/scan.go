...
func init() {
	...
	_ = scanCmd.Flags().String("output-target", "", "Set the output target for the asset report: an AWS SQS topic URL, a Splunk HEC URL, a local file, or a local directory (the OCSF formats write one file per event class into it)")
