...
func TestASecretIsSentToTheProviderAndNotToArgv(t *testing.T) {
	...
	if got := req.Flags["password"]; got == nil || string(got.Value) != "must-never-reach-argv" {
		t.Fatalf("the password did not reach the provider: %+v", req.Flags)
	}
	for _, arg := range f.Args() {
		if arg == "must-never-reach-argv" || arg == "--password" {
	...
	cases := []struct {
	...
	}{
		{
			...
			conn: &inventory.Config{Credentials: []*vault.Credential{
				{Type: vault.CredentialType_password, Password: "must-never-reach-argv"}}},
		...
		},
		{
			...
			conn: &inventory.Config{Credentials: []*vault.Credential{
				{Type: vault.CredentialType_password, User: "must-never-reach-argv"}}},
			want: PlacedCredential,
		},
		{
			name: "a bearer token in Secret",
			conn: &inventory.Config{Credentials: []*vault.Credential{
				{Type: vault.CredentialType_bearer, Secret: []byte("must-never-reach-argv")}}},
			want: PlacedCredential,
		},
		{
			// openai, ollama, huggingface and claude never read Credentials.
			name: "a connection option, which no vault reference reaches",
			conn: &inventory.Config{Options: map[string]string{"api-key": "must-never-reach-argv"}},
			want: PlacedOption,
		},
		{
		...
		},
		{
			...
			conn: &inventory.Config{
				Options:     map[string]string{"password": "must-never-reach-argv"},
				Credentials: []*vault.Credential{{Password: "must-never-reach-argv"}},
			},
			want: PlacedCredential,
		},
	}
