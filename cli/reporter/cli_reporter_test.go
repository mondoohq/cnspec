...
// TestCompactReporterWarningsSpacing guards against the warnings block not
// calling beginAssetSection(): r.assetBodyPrinted stayed false after
// printing a scan warning (e.g. a crashed provider), so the next section
// treated itself as the first body section and skipped its separator,
// leaving a double blank line between the warning and that section.
func TestCompactReporterWarningsSpacing(t *testing.T) {
	assetMrn := "//assets.api.mondoo.app/spaces/test/assets/crashed-provider"
	riskMrn := "//policy.api.mondoo.app/risks/example"
	...
	data := &policy.ReportCollection{
		...
		Warnings: map[string]*policy.ScanWarnings{
			assetMrn: {Messages: []string{"the 'os' provider crashed: connection refused"}},
		},
		Reports: map[string]*policy.Report{
			assetMrn: {
				ScoringMrn: assetMrn,
	...
	strData := buf.String()
	require.Contains(t, strData, "the 'os' provider crashed: connection refused")

	lines := strings.Split(strData, NewLineCharacter)

	warningIdx := -1
	for i, l := range lines {
		if strings.Contains(l, "the 'os' provider crashed: connection refused") {
			warningIdx = i
			break
		}
	}
	require.GreaterOrEqual(t, warningIdx, 0, "expected the warning line to be printed")
	...
	require.Greater(t, risksIdx, warningIdx, "expected the Risks section after the warning")

	// Exactly one blank line separates the warning block from the next
	// section: the line right before "Risks" is blank, the one before that
	// is not (the warning message itself).
	assert.Equal(t, "", lines[risksIdx-1], "expected a single blank line before the Risks section")
	assert.NotEqual(t, "", lines[risksIdx-2], "expected exactly one blank line (no double gap) after the warning")
}
