...
const (
	// 1 asset, 1 report, 0 errors -- the single-asset happy path, carrying a
	// bundle, resolved policies, raw llx data and a vuln report.
	fixtureUbuntu = "./testdata/report-ubuntu.json"
	// 15 assets, 0 reports, 15 errors -- every asset failed to scan. A viewer
	// must be able to tell this apart from "assets with no findings", so the
	// artifact has to carry the errors map through untouched.
	fixtureK8s = "./testdata/report-k8s.json"
)
...
func TestCollectionKeepsFullFidelity(t *testing.T) {
	orig := loadFixture(t, fixtureUbuntu)
...
func TestCollectionDoesNotMutateReceiver(t *testing.T) {
	orig := loadFixture(t, fixtureUbuntu)
	before := loadFixture(t, fixtureUbuntu)
