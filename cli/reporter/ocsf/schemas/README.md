...
1. **Code generation.** `internal/gen` reads it to generate `types.gen.go` and `enums.gen.go`
   (`go generate ./cli/reporter/ocsf/...`), so every attribute name, Go type, enum value and
   caption comes from the schema rather than from a person.
2. **Validation.** `TestOcsfSchemaValidation` (`cli/reporter/ocsf_validate_test.go`) checks
