...
func BenchmarkOcsfConvert(b *testing.B) {
	...
	conf := ocsfConfig{version: ocsf.DefaultVersion, findings: OcsfFindingsCompliance}
...
func BenchmarkOcsfConvertBareChecks(b *testing.B) {
	...
	b.ReportAllocs()
	for b.Loop() {
		if _, err := convertToOCSF(report, conf, fixedScanTime); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkOcsfStreamParquet(b *testing.B) {
	...
	for b.Loop() {
		...
		if err := StreamOCSF(report, conf, w); err != nil {
			b.Fatal(err)
		}
	}
}
