...
func (c *ocsfConverter) detectionFinding(resolved *policy.ResolvedPolicy, report *policy.Report, query *policy.Mquery, score *policy.Score, ctx *ocsfAssetContext) ocsf.DetectionFinding {
	...
	// ocsfCheckSeverity and checkUnmapped already guard on Type for the same
...
// ocsfRiskLevel maps a cnspec risk value to an OCSF risk_level_id. Same bands as
// the severity mapping -- the boundaries come from reportdoc -- but risk_level is
// a separate enum with its own names, and it calls the lowest band Info.
...
// ocsfImpactLevel maps a check's configured impact to an OCSF impact_id. Impact
// runs on the same 0-100 cnspec scale as risk, so it shares the boundaries;
// OCSF's impact enum has no informational band and calls its floor Unknown.
