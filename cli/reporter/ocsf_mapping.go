...
// maxAssessmentBytes is the same cap under the name the assessment path uses.
const maxAssessmentBytes = maxUnmappedBytes
...
func ocsfCheckSeverity(score *policy.Score) int {
	...
	case policy.ScoreType_Result:
		...
		return ocsfSeverityFromRisk(reportdoc.ScoreRisk(score))
	default:
		return ocsf.SeverityInformational
	}
}
...
// The band boundaries come from reportdoc so they cannot drift away from the
// severity cnspec prints; only the names are OCSF's. OCSF calls the lowest band
// Informational, which is also what CVSS and Splunk consumers expect there.
