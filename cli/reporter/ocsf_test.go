...
func TestOcsfFileHandlerDirectory(t *testing.T) {
	for _, tc := range []struct {
		...
		// A trailing separator is how a target that does not exist yet asks to be
		// a directory; see isDirTarget. Parquet is always per-class, so it needs
		// one whatever the target looks like.
...
// TestOcsfFileHandlerExtensionlessTargetIsAFile pins the case the two isDirTarget
// copies used to answer differently. A target that does not exist and has no
// extension is a file, so ocsf-json writes one NDJSON stream to it rather than
// creating a directory named after it. ocsf-parquet is per-class by nature and
// takes the directory branch regardless.
func TestOcsfFileHandlerExtensionlessTargetIsAFile(t *testing.T) {
	target := filepath.Join(t.TempDir(), "results")
	conf := defaultPrintConfig()
	conf.format = FormatOcsfJson

	require.NoError(t, (&ocsfFileHandler{target: target, conf: conf}).
		WriteReport(t.Context(), advisoryReportCollection()))

	info, err := os.Stat(target)
	require.NoError(t, err)
	require.False(t, info.IsDir(), "an extensionless target that does not exist is a file")

	raw, err := os.ReadFile(target)
	require.NoError(t, err)
	assert.Len(t, strings.Split(strings.TrimSpace(string(raw)), "\n"), 5,
		"every class goes into the one stream")
}

// TestOcsfFileHandlerRemovesStaleClassFiles covers reusing an output directory.
//
...
func TestOcsfFileHandlerRemovesStaleClassFiles(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "out") + string(os.PathSeparator)

	detection := defaultPrintConfig()
	detection.format = FormatOcsfJson
	detection.ocsfFindings = OcsfFindingsDetection
	require.NoError(t, (&ocsfFileHandler{target: dir, conf: detection}).
		WriteReport(t.Context(), advisoryReportCollection()))
	...
	compliance := defaultPrintConfig()
	compliance.format = FormatOcsfJson
	compliance.ocsfFindings = OcsfFindingsCompliance
	require.NoError(t, (&ocsfFileHandler{target: dir, conf: compliance}).
		WriteReport(t.Context(), advisoryReportCollection()))
...
func TestOcsfDetectionFindings(t *testing.T) {
	...
	all, err := convertToOCSF(reportfixture.Sample(),
		ocsfConfig{version: ocsf.DefaultVersion, findings: OcsfFindingsDetection}, fixedScanTime)
	require.NoError(t, err)
