...
// reporter.ScoreStatusLabel, which is the single place in cnspec that collapses
// the seven score types into an outcome.
//
...
func StatusOf(score *policy.Score) Status {
	return Status(reporter.ScoreStatusLabel(score))
}

// Icon is the emoji for a status. It matches reporter.ScoreStatusIcon.
...
func newBuilder(collection *policy.ReportCollection) *builder {
	...
	b.queriesByCodeID = reporter.DeterministicQueryMap(b.bundle)
...
func (b *builder) buildAsset(mrn string) *Asset {
	...
	asset.platformKeys = reporter.PlatformRemediationKeys(asset.Platform)
...
func (b *builder) buildCheck(asset *Asset, job *policy.ReportingJob, query *policy.Mquery) *Check {
	...
	check.Impact, check.HasImpact = reporter.QueryImpact(query)
	check.Severity = reporter.RiskSeverityLabel(check.Impact)
	return check
}
