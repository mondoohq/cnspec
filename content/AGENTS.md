...
- **A list of filters is an OR, not an AND.** `filters:` written as `- mql: a` / `- mql: b` applies the check when *either* item matches: the resolver takes the first hit (`policy.Filters.Supports`). It reads like "this asset type, and this condition", which is exactly how it gets misused, and then the broad item matches alone and the narrowing one never takes effect. Write an AND as one expression with a trailing `&&`, and a real OR with an explicit `||`. `content/validation/filters` rejects the list form outright.
- **A check that can only read its data as root runs only in a root scan.** Some sources are closed to other users: `/etc/sudoers`, a 0600 config file, `systemsetup`, `profiles`. In a non-root scan the check can't reach a verdict and scores `error`. Filter it with `command("id -u").stdout.trim == 0` so a non-root scan skips it instead. Don't filter on the file existing: it exists, it just isn't readable.
...
MQL resource links, built-in functions, and the authoring guide are in the repo-root [`AGENTS.md`](../AGENTS.md), which loads alongside this file.
