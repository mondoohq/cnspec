...
- **Kubernetes Network Posture** - `mondoo-kubernetes-network-posture.mql.yaml` - Kubernetes Service, Ingress, Gateway API, egress, NAT, HBN, MultiNetworkPolicy, AdminNetworkPolicy, Calico, and Cilium network posture visibility; use cluster discovery when testing it directly, for example: `cnspec scan k8s --discover clusters --policy-bundle content/mondoo-kubernetes-network-posture.mql.yaml`
