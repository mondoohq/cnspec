...
Targets: `aws`, `azure`, `oci`, `gcp`, `digitalocean`, `nutanix`, `alicloud`, `vercel`, the Cobra CLIs (`kubernetes`, `github`, `gitlab`, `hetzner`, `databricks`), and the REST APIs (`cloudflare`, `tailscale`, `slack`, `atlassian`, `grafana`, `mongodbatlas`, `okta`, `portainer`).
