...
- Nothing in the harness invokes a shell. Files go into a container with `docker cp` rather than a piped `sh -c`, so no fixture path or file content can be read as syntax.
