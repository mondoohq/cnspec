...
- Fixture account passwords are generated per run, never written down. A password in this directory is a credential in the repository, and a scanner cannot tell a throwaway from a real one — so an allowlist entry that silenced it would also hide the next real one.
- Nothing in the harness invokes a shell, and everything it can execute is on one allowlist (`docker`, `cnspec`, `openssl`) enforced at a single choke point. Files go into a container with `docker cp` rather than a piped `sh -c`, so no fixture path or file content can be read as syntax. The fixtures are checked-in code rather than input, so this is a boundary that keeps that true rather than a defence against them.
