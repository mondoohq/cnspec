...
#   - "file": checked into data/ because the vendor serves the spec
#     from a live, unversioned endpoint that can change (or break)
#     without notice. Regenerate with upstream/dump/api_specs.py; never hand-edit.
...
CLOUDFLARE_OPENAPI_SHA = "f2df0ca75c0fec9047e91a34c20dd666c2dcd1ba"
SLACK_OPENAPI_SHA = "bc08db49625630e3585bf2f1322128ea04f2a7f3"
GRAFANA_OPENAPI_SHA = "dbcfc2c152c80947649f8725eba3a73d51b84c69"
MONGODBATLAS_OPENAPI_SHA = "cc6b97d0a3a518eece4544f1007677449f9ac2b6"
    ...
    if "file" in source:
        ...
        if not path.exists():
            print(
                ...
                "Regenerate it with:\n"
                f"  python3 {DUMP_DIR / 'api_specs.py'}",
                file=sys.stderr,
            )
            sys.exit(1)
...
API_PROVIDERS = {
    ...
    "tailscale": {
        ...
        # unstable, so the spec is checked in via upstream/dump/api_specs.py.
    ...
    "atlassian": {
        ...
        # upstream/dump/api_specs.py. The org spec mounts at /admin, the
    ...
    "vercel": {
        ...
        # checked in via upstream/dump/api_specs.py.
        #
...
def validate_api_provider(key: str) -> tuple[int, int]:
    ...
    for policy_name in provider["policies"]:
        policy_file = CONTENT_DIR / policy_name
