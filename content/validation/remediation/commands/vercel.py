...
# grammar is generated once by upstream/dump/vercel.py and checked into
# data/vercel_commands.json — the same model as azure_commands.json and
...
VERCEL_POLICY_FILE = CONTENT_DIR / "mondoo-vercel-security.mql.yaml"
VERCEL_COMMANDS_FILE = DATA_DIR / "vercel_commands.json"
...
def _load_vercel_db() -> tuple[dict, dict]:
    ...
    if not VERCEL_COMMANDS_FILE.exists():
        print(
            ...
            f"Run upstream/dump/vercel.py first to generate it.",
            file=sys.stderr,
        )
        sys.exit(1)
