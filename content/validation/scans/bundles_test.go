...
func TestNetworkPostureContent(t *testing.T) {
	...
	t.Run("policy compiles network posture controls", func(t *testing.T) {
		...
		assert.Contains(t, queries["mondoo-kubernetes-network-posture-internet-exposure-evidence"], `metadataClassification != empty && owner != empty`)
		...
		assert.Contains(t, queries["mondoo-kubernetes-network-posture-public-egress-classified"], `sourceRef != empty`)
		assert.Contains(t, queries["mondoo-kubernetes-network-posture-public-egress-classified"], `metadataClassification != empty && owner != empty`)
