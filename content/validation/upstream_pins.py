...
#
#   check_upstream_versions.py   reports which pins are behind
#   bump_upstream_versions.py    rewrites the ones that are
#
...
#   tflint-ruleset
#   terraform-provider
#                 content/validation/validate_terraform_remediation.py
#                 TFLINT_PLUGIN_MAP and PROVIDER_MAP
#   spec          content/validation/validators/openapi.py
#                 the `*_OPENAPI_SHA` commit pins
#   grammar       content/validation/cmd_data/*.json
...
TERRAFORM_VALIDATOR = SCRIPT_DIR / "validate_terraform_remediation.py"
...
# ---------------------------------------------------------------------------
# Terraform tooling declared in validate_terraform_remediation.py
# ---------------------------------------------------------------------------
...
def check_grammars() -> list[Pin]:
    ...
    # than in-process here. Never hand-edit the files in cmd_data/.
    return [
        Pin(
...
def sync_dump_script_pins() -> list[str]:
    ...
    if version and script.exists():
        ...
        if re.search(pattern, text) and not re.search(rf'VERCEL_VERSION = "{re.escape(version)}"', text):
            ...
            changed.append(f"dump_vercel_commands.py VERCEL_VERSION -> {version}")
    return changed
