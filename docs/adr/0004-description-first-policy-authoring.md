...
- `internal/generate/` — the shared, backend-agnostic core:
  ...
  - `target.go` — provider resolution from `filters:` (with hyphenated-platform
    and UID fallbacks).
  - `validate.go` — in-process `mqlc.Compile` gate mirroring the linter, plus
    the execute-and-assert gate (`QueryRunner` + `executeValidator`).
...
- **No secrets added by cnspec.** cnspec passes no credentials to the agent and
  stores none; the agent uses its own configured auth. The prompt is passed as a
  subprocess argument (no shell), so there is no shell-injection surface from
  prompt content.
