...
What cnspec offers instead is core OCSF, in the class Splunk models on:
...
Transport is deliberately not part of this. cnspec writes the events to a file or a directory
and stops there; delivery to an external destination belongs in Mondoo Platform, which already
owns it and hardens the egress path a tenant-supplied destination URL needs. An HTTP Event
Collector output target was prototyped in cnspec and removed for that reason: it would have
been a second, weaker HEC client reachable from a URL typed on the command line.
...
Those figures are the **JSON** path, which is the one that is genuinely flat: it encodes each
asset's events and lets them go, so peak tracks the largest asset rather than the scan.
Streamed Parquet does not behave that way and it is worth being explicit about, because
`content/README.md` recommends `ocsf-parquet` at scale. The Parquet writer buffers a row group
and a page per column before it can flush anything, so peak heap above baseline measured
+108.9 MB at 10k rows and +339.7 MB at 100k rows, against +5.5 MB and +26.4 MB for streamed
JSON on the same scans. That is bounded rather than unbounded -- it is buffer size, not a leak
-- but it is three orders of magnitude above the JSON figure and should not be read off the
sentence above.

Lowering `maxRowsPerRowGroup` is the obvious lever and mostly is not one: at 5k rows per group
the same scans measured +123.2 MB and +215.6 MB, better at 100k rows, worse at 10k, and nowhere
near the JSON path. The floor is `maxDataPageSize` times the column count of a finding class,
which no row count gets below. It stays at 50k, which is the value that keeps a row group
comfortably inside Security Lake's ceiling.
