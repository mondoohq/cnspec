...
<Callout>
  If you install cnspec in an environment where it can't automatically update
  providers, you're responsible for installing provider updates.
</Callout>
To prevent error messages from displaying when you run commands in read-only
mode, turn off provider auto-update. To learn how, read [Turn off provider
auto-update](#turn-off-provider-auto-update) below.
...
<Callout>
  If you install cnspec in an air-gapped environment, you're responsible for
  installing provider updates.
</Callout>
To prevent error messages from displaying when you run commands, turn off
...
<Callout>
  If you disable auto-update, be sure to check regularly for new versions of
  providers. To learn more, read [Install, update, and remove
  providers](#install-update-and-remove-providers) below.
</Callout>
...
### Providers and release channels

Providers follow the same release channel as cnspec itself. With
`update_channel: preview` set, `cnspec providers update` resolves pre-release
provider versions; with `stable` it resolves stable ones. With the setting
absent, providers follow the cnspec build you are running — a pre-release
binary resolves pre-release providers on its own. See
[Update cnspec](/cnspec/cnspec-adv-install/update/).

This matters because a provider is built against a specific cnspec schema.
Running a pre-release cnspec against stable providers can produce empty results
that look like passing checks rather than an error.

A provider with no pre-release published falls back to its stable version, so
switching channels never leaves a provider unavailable.
