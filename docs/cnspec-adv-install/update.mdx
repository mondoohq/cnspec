...
If you don't set `update_channel` at all, cnspec follows the build it is running:
a pre-release binary resolves pre-release providers, and a stable binary resolves
stable ones. So installing a release candidate is enough — you don't have to
configure the channel as well, and a stable install is never moved onto
pre-releases by anything but an explicit setting.
