...
For policy *content* authoring (writing `.mql.yaml` files), see `content/AGENTS.md`.
...
Same as the root `AGENTS.md` — never hand-edit generated files (`*.pb.go`, `*.ranger.go`, `*.vtproto.pb.go`, `*_gen.go`); regenerate with `make cnspec/generate`.
