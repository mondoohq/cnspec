...
func (s *LocalScanner) distributeJob(job *Job, ctx context.Context, upstream *upstream.UpstreamConfig) (*ScanResult, error) {
	...
	// Make sure the resource schemas of all providers required by the bundle
	// are installed before we scan. We only check here for bundles fetched from
	// the server. Local policy bundles already have their requirements ensured
	// before compilation in loadPolicies(). Binaries are deliberately not
	// pulled here: which providers a scan needs is only known once an asset's
	// filters have run (see runPolicy), and eagerly installing every required
	// provider breaks scanning on platforms a provider does not ship binaries
	// for. A failure only affects the policies of that provider, so it must
	// not abort the scan.
...
func (sc *scanContext) scanSubtree(ctx context.Context, node *discovery.TrackedAsset) error {
	...
	for _, child := range node.Children {
		...
		if len(connected.Children) > 0 {
			...
			// We drain before recursing to avoid keeping data from multiple
			// subtrees (e.g. namespaces) in memory simultaneously.
			<-sc.connSem
...
func (s *LocalScanner) upstreamClient(ctx context.Context, conf *upstream.UpstreamConfig) (*upstream.UpstreamClient, error) {
	s.upstreamOnce.Do(func() {
		s._upstreamClient, s.upstreamErr = conf.InitClient(ctx)
	})
	return s._upstreamClient, s.upstreamErr
}
...
func (s *localAssetScanner) prepareAsset() error {
	...
	// installed before we try to compile it. This handles bundles with Require
	// metadata. Binaries are installed later, in runPolicy, once the asset's
