...
func NewSqliteScanDataStore(filePath, assetMrn, uploadSessionId string) (*SqliteScanDataStore, error) {
...
func (s *SqliteScanDataStore) initializeDatabase() error {
	...
	s.dataStmt, err = s.db.Prepare(`
		INSERT INTO data (code_id, data) VALUES (?, ?)
	`)
	if err != nil {
...
func (s *SqliteScanDataStore) StreamData(ctx context.Context, callback func(string, *llx.Result) error) error {
	rows, err := s.db.Query("SELECT code_id, data FROM data ORDER BY id")
	if err != nil {
...
func (s *SqliteScanDataStore) scanScore(rows *sql.Rows) (*Score, error) {
	score := &Score{}
	var riskFactors, sources []byte
...
func (s *SqliteScanDataStore) ValidateFile() error {
	...
	tables := []string{"metadata", "scores", "data"}
