...
func toOcsf(t *testing.T, r *policy.ReportCollection) *ocsf.Events {
	t.Helper()
	events, err := convertAt(r, Options{Version: ocsf.DefaultVersion, Findings: ocsf.FindingsCompliance}, fixedScanTime)
...
func TestOcsfConverter(t *testing.T) {
	events := toOcsf(t, reportfixture.Sample())
...
func TestOcsfVulnerabilityFindings(t *testing.T) {
	...
	noCVE.VulnReports[reportfixture.AssetMrn].Advisories[0].Cves = nil
	...
	events19, err := convertAt(noCVE, Options{Version: ocsf.Version190, Findings: ocsf.FindingsCompliance}, fixedScanTime)
	require.NoError(t, err)
...
func TestOcsfConverterIsDeterministic(t *testing.T) {
	first := bytes.Buffer{}
	second := bytes.Buffer{}
	require.NoError(t, toOcsf(t, reportfixture.Sample()).WriteJSON(&first))
	require.NoError(t, toOcsf(t, reportfixture.Sample()).WriteJSON(&second))
...
func TestOcsfComplianceStandardsFallback(t *testing.T) {
	...
	bare := toOcsf(t, reportfixture.Detailed())
...
func TestOcsfVersionSelection(t *testing.T) {
	...
	v13, err := convertAt(report, Options{Version: ocsf.Version130, Findings: ocsf.FindingsCompliance}, fixedScanTime)
	require.NoError(t, err)
	...
	v19, err := convertAt(report, Options{Version: ocsf.Version190, Findings: ocsf.FindingsCompliance}, fixedScanTime)
	require.NoError(t, err)
...
func TestConvertToDir(t *testing.T) {
	for _, tc := range []struct {
		...
		{EncodingJSON, ".jsonl"},
		{EncodingParquet, ".parquet"},
	} {
		dir := filepath.Join(t.TempDir(), "out")
		written, err := ConvertToDir(advisoryReportCollection(), dir, Options{}, tc.encoding)
		require.NoError(t, err)
...
// TestConvertToDirRemovesStaleClassFiles covers reusing an output directory.
//
// A run creates a file only for a class that produced events, and os.Create
// truncates only what it opens, so a run producing fewer classes than the last
// one leaves the previous run's files behind. Switching ocsf-findings from
// detection to compliance is the ordinary way to do that, and it leaves
// detection_finding.jsonl beside the new compliance_finding.jsonl: a crawler over
// the directory then reads both and counts every check twice.
func TestConvertToDirRemovesStaleClassFiles(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "out")

	_, err := ConvertToDir(advisoryReportCollection(), dir,
		Options{Findings: ocsf.FindingsDetection}, EncodingJSON)
	require.NoError(t, err)

	stale := filepath.Join(dir, ocsf.ClassDetectionFinding+".jsonl")
	require.FileExists(t, stale, "the detection run writes class 2004")

	// A file cnspec does not write is not cnspec's to delete.
	unrelated := filepath.Join(dir, "notes.txt")
	require.NoError(t, os.WriteFile(unrelated, []byte("keep me"), 0o600))

	_, err = ConvertToDir(advisoryReportCollection(), dir,
		Options{Findings: ocsf.FindingsCompliance}, EncodingJSON)
	require.NoError(t, err)

	assert.FileExists(t, filepath.Join(dir, ocsf.ClassComplianceFinding+".jsonl"))
	assert.NoFileExists(t, stale,
		"the previous run's class file would double-count every check")
	assert.FileExists(t, unrelated, "only the names cnspec writes are swept")
}

// TestConvertMixesEveryClassIntoOneStream is the other shape: a single
// newline-delimited document, which is what a SIEM ingesting one file expects.
func TestConvertMixesEveryClassIntoOneStream(t *testing.T) {
	buf := bytes.Buffer{}
	require.NoError(t, Convert(advisoryReportCollection(), &buf, Options{}))

	lines := strings.Split(strings.TrimSpace(buf.String()), "\n")
	assert.Len(t, lines, 5, "3 checks + 1 vulnerability + 1 inventory event, all in one stream")
}

// TestOptionsZeroValue pins that the exported Options is usable unset. An empty
// Version would otherwise reach metadata.version, where it passes no validator
// anywhere, and an empty Findings would report checks as no class at all.
func TestOptionsZeroValue(t *testing.T) {
	events, err := convertAt(reportfixture.Sample(), Options{}, fixedScanTime)
	require.NoError(t, err)

	require.NotEmpty(t, events.ComplianceFindings, "compliance is the default class")
	assert.Empty(t, events.DetectionFindings)
	assert.Equal(t, string(ocsf.DefaultVersion), events.ComplianceFindings[0].Metadata.Version)
}

func TestOcsfMillis(t *testing.T) {
	assert.Equal(t, int64(0), millis(0))
	assert.Equal(t, int64(1700000000000), millis(1700000000), "seconds are scaled up")
	assert.Equal(t, int64(1700000000000), millis(1700000000000), "milliseconds are left alone")
}

func TestOcsfParseTime(t *testing.T) {
	assert.Equal(t, int64(0), parseTime(""))
	assert.Equal(t, int64(0), parseTime("not a date"), "an unparsable date must not become the year 1")
	assert.Equal(t, time.Date(2023, 1, 2, 3, 4, 5, 0, time.UTC).UnixMilli(), parseTime("2023-01-02T03:04:05Z"))
	assert.Equal(t, time.Date(2023, 1, 2, 0, 0, 0, 0, time.UTC).UnixMilli(), parseTime("2023-01-02"))
}

func TestConvertVulnReport(t *testing.T) {
...
func erroredReportCollection() *policy.ReportCollection {
	...
	report.Errors = map[string]string{reportfixture.AssetMrn: "could not connect to the asset"}
	return report
}
...
func TestOcsfDetectionFindings(t *testing.T) {
	...
	all, err := convertAt(reportfixture.Sample(),
		Options{Version: ocsf.DefaultVersion, Findings: ocsf.FindingsDetection}, fixedScanTime)
	require.NoError(t, err)
	...
	// Only a check that produced a verdict carries risk. score.Value is 0 for an
	// errored or skipped check, so deriving risk from it unguarded reports both as
	// risk_score 100 / Critical -- and risk_score is what Splunk ES Risk-Based
	// Alerting sums per asset, so every check that could not apply to a host would
	// bill that host 100 points. The fixture has one of each outcome, which is the
	// point of asserting over all three rather than the failing one.
	for _, finding := range all.DetectionFindings {
		if finding.StatusCode == "PASS" {
			assert.Equal(t, 0, finding.RiskScore, "a passing check is risk 0")
			assert.Equal(t, ocsf.RiskLevelInfo, finding.RiskLevelID)
			continue
		}
		assert.Equal(t, 0, finding.RiskScore,
			"%s carries no verdict, so it carries no risk score", finding.StatusCode)
		assert.Equal(t, ocsf.RiskLevelInfo, finding.RiskLevelID,
			"%s carries no verdict, so it is not a Critical risk", finding.StatusCode)
		assert.Equal(t, "Info", finding.RiskLevel)
		assert.NotContains(t, finding.Unmapped, "risk",
			"unmapped[risk] is already guarded on score type; risk_score must agree with it")
	}
}

// TestOcsfEnumZeroSurvivesJSON pins that an enum identifier whose value is 0
// reaches the JSON, because for most OCSF enums 0 is a meaningful value rather
// than an absent one: compliance.status_id 0 is Unknown, risk_level_id 0 is Info.
//
// These fields used to carry omitempty, generated from schema optionality --
// which says whether a producer must supply a value, not which values mean
// something. The effect was that the errored outcome this converter goes out of
// its way to distinguish vanished from the document: status "Unknown" with no
// status_id beside it, indistinguishable to a consumer switching on the id from a
// field cnspec never populated. It also split the two output formats, since the
// Parquet writer has no such rule and wrote the 0 either way.
//
// Asserting on the serialized bytes is the point. The struct is identical in both
// cases, so a test reading finding.Compliance.StatusID passes with or without the
// bug.
func TestOcsfEnumZeroSurvivesJSON(t *testing.T) {
	report := reportfixture.Detailed()
	// an errored check: no verdict, so compliance.status_id is Unknown (0)
	for _, r := range report.Reports {
		for id := range r.Scores {
			r.Scores[id].Type = policy.ScoreType_Error
		}
	}

	events := toOcsf(t, report)
	require.Len(t, events.ComplianceFindings, 1)
	require.Equal(t, ocsf.ComplianceStatusUnknown, events.ComplianceFindings[0].Compliance.StatusID,
		"an errored check has no compliance verdict")

	var buf bytes.Buffer
	require.NoError(t, events.WriteJSON(&buf))

	// WriteJSON emits newline-delimited events; the compliance finding is the one
	// carrying a compliance object.
	var compliance map[string]any
	for _, line := range strings.Split(strings.TrimSpace(buf.String()), "\n") {
		var doc map[string]any
		require.NoError(t, json.Unmarshal([]byte(line), &doc))
		if c, ok := doc["compliance"].(map[string]any); ok {
			compliance = c
			break
		}
	}
	require.NotNil(t, compliance, "the compliance object is present")
	require.Contains(t, compliance, "status_id",
		"status_id 0 is Unknown, a verdict cnspec means; omitting it loses that")
	assert.EqualValues(t, ocsf.ComplianceStatusUnknown, compliance["status_id"])
	assert.Equal(t, "Unknown", compliance["status"], "the caption and the id have to agree")
}

func TestTruncateAssessment(t *testing.T) {
	assert.Equal(t, "short", truncateAssessment("short"))

	long := strings.Repeat("a", maxUnmappedBytes+100)
	...
	runes := strings.Repeat("ü", maxUnmappedBytes)
...
func TestOcsfStreamedParquetAcrossAssets(t *testing.T) {
	...
	_, err := ConvertToDir(largeReportCollection(assets, checks), dir, Options{}, EncodingParquet)
	require.NoError(t, err)
