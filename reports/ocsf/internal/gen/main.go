...
//
// Run it with `go generate ./reports/ocsf/...`.
//
...
type field struct {
	...
	IsArray  bool
	IsMap    bool
	// IsEnum marks a scalar whose value is an OCSF enum identifier. Zero is a
	// meaningful value for most of them, so they never take omitempty; see
	// writeFields.
	IsEnum   bool
	Doc      string
	Desc     string
...
func writeFields(b *strings.Builder, fields []field) {
	for i, f := range fields {
		...
		// omitempty follows schema optionality, except for enum identifiers.
		// Optionality is about whether the producer has to supply a value; it says
		// nothing about which values are meaningful. For an enum id, zero usually
		// is one: compliance.status_id 0 is Unknown, risk_level_id 0 is Info,
		// impact_id 0 is Unknown. Omitting those turns "cnspec assessed this as
		// Unknown" into "cnspec said nothing", which a consumer switching on the id
		// cannot tell apart -- and it made JSON and Parquet disagree about the same
		// event, since the Parquet writer has no such rule and writes the 0.
		jsonTag := f.OCSFName
		if (f.Optional && !f.IsEnum) || f.IsArray || f.IsMap {
			jsonTag += ",omitempty"
		}
...
func writeHeader(b *strings.Builder, name string) {
	...
	b.WriteString("// Code generated by reports/ocsf/internal/gen from the compiled OCSF\n" +
