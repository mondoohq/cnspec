...
// platform stores each SBOM and enriches it into vulnerabilities itself -- the
// client computes no VEX. createAssets lets the platform create assets it does
// not yet know, from the SBOM's own asset identification. Returns the number of
// SBOMs stored.
//
// This is the "store and auto-enrich" path, distinct from ScanSBOM ("scan for
// results"), which returns VEX for the caller to display or upload.
